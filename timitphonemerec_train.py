...
VAL_BATCH_SIZE = 400
NUM_OPT_STEPS = 2000
NUM_STEPS_PER_TRAIN_SUMMARY = 1
NUM_STEPS_PER_VAL_SUMMARY = 10
...
def main():
  """ Train an RNN for TIMIT phoneme recognition. """
  ...
  train_inputs, train_labels, val_inputs, val_labels = timitphonemerec.load_split(args.data_dir, val=True,
                                                                                  mfcc=True, normalize=True)
  if args.debug:
    train_inputs = [seq[::16] for seq in train_inputs]
    train_labels = [seq[::16] for seq in train_labels]
    val_inputs = [seq[::16] for seq in val_inputs]
    val_labels = [seq[::16] for seq in val_labels]
  ...
  train_labels = [seq[:, np.newaxis] for seq in train_labels]
  val_labels = [seq[:, np.newaxis] for seq in val_labels]

  train_batches = utils.full_bptt_batch_generator(train_inputs, train_labels, BATCH_SIZE, shuffle=True)
  ...
  for step in range(NUM_OPT_STEPS):
    ...
    sess.run(optimize_op, feed_dict={model.inputs: batch_inputs,
    ...
    if step % NUM_STEPS_PER_VAL_SUMMARY == 0:
      ...
      val_batches = utils.full_bptt_batch_generator(val_inputs, val_labels, VAL_BATCH_SIZE,
